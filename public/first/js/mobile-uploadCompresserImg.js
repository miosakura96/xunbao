 ...
 * @description
 * 作者: 1129421419(119421419@qq.com)
...
//------------------------------------------------------------------------------
var objGlobal = {                                                               
    checkUA: function() {                                                       //判断手机系统，微信版本等信息
        var mb_os = {ios : false, android: false, version: false},
            mb_br = {},
            mb_ua = navigator.userAgent;

            mb_br.weixin = mb_ua.match(/MicroMessenger\/([^\s]+)/),
            mb_br.webkit = mb_ua.match(/WebKit\/([\d.]+)/),
            mb_br.android = mb_ua.match(/(Android)\s+([\d.]+)/),
            mb_br.ipad = mb_ua.match(/(iPad).*OS\s([\d_]+)/),
            mb_br.ipod = mb_ua.match(/(iPod).*OS\s([\d_]+)/),
            mb_br.iphone = !mb_br.ipod && !mb_br.ipad && mb_ua.match(/(iPhone\sOS)\s([\d_]+)/),
            mb_br.webos = mb_ua.match(/(webOS|hpwOS)[\s\/]([\d.]+)/),
            mb_br.touchpad = mb_br.webos && mb_ua.match(/TouchPad/),
            mb_br.kindle = mb_ua.match(/Kindle\/([\d.]+)/),
            mb_br.silk = mb_ua.match(/Silk\/([\d._]+)/),
            mb_br.blackberry = mb_ua.match(/(BlackBerry).*Version\/([\d.]+)/),
            mb_br.mqqbrowser = mb_ua.match(/MQQBrowser\/([\d.]+)/),
            mb_br.chrome = mb_ua.match(/CriOS\/([\d.]+)/),
            mb_br.opera = mb_ua.match(/Opera\/([\d.]+)/),
            mb_br.safari = mb_ua.match(/Safari\/([\d.]+)/);

            if (mb_br.weixin) {
               mb_os.wx = true;
               mb_os.wxVersion = mb_br.weixin[1];
            }
            if (mb_br.android) {
                mb_os.android = true;
                mb_os.version = mb_br.android[2];
            }
            if (mb_br.iphone) {
                mb_os.ios = mb_os.iphone = true;
                mb_os.version = mb_br.iphone[2].replace(/_/g, '.');
            }
            if (mb_br.ipad) {
                mb_os.ios = mb_os.ipad = true;
                mb_os.version = mb_br.ipad[2].replace(/_/g, '.');
            }
            if (mb_br.ipod) {
                mb_os.ios = mb_os.ipod = true;
                mb_os.version = mb_br.ipod[2].replace(/_/g, '.');
            }
        return mb_os;
    },
    DIC: {                                                                      //弹窗提示
        dialog: function(opts) {
            setTimeout(function() {
                objGlobal.DIC._dialog(opts);
            }, 5);
        },
        _dialog: function(opts) {                                               //objGlobal.DIC.dialog({title:"提示信息",content:'图片大小超过', autoClose:false, cancelValue:"取消", okValue:"确定", isMask:true,cancel:function() {},ok:function() {},id:"tip"});
            var opts = opts || {};
            ...
            var existDialogCount = $('div[id^="fwin_dialog_"]').length || 0;
            ...
            if (isHtml) {
                ...
                if (!opts.title && !btnOk && !btnCancel) {
                    dialogHtmlArr.push('<div style="position:fixed;z-index:' + dialogZIndex +';" id="' + dialogId + '"><div class="tip_tips br5">');
                    if (dId == 'loading') {
                        // loading动画
                        dialogHtmlArr.push('<div class="tip_loadicon tipL" style="vertical-align: -5px;"><span class="tip_blockG" id="tip_rotateG_01"></span><span class="tip_blockG" id="tip_rotateG_02"></span><span class="tip_blockG" id="tip_rotateG_03"></span><span class="tip_blockG" id="tip_rotateG_04"></span><span class="tip_blockG" id="tip_rotateG_05"></span><span class="tip_blockG" id="tip_rotateG_06"></span><span class="tip_blockG" id="tip_rotateG_07"></span><span class="tip_blockG" id="tip_rotateG_08"></span></div> ');
                    }
                    ...
                    dialogHtmlArr.push('<div style="min-width:350px;position:fixed;z-index:' + dialogZIndex +';" id="' + dialogId + '"><span class="tip_close"></span>');
                    dialogHtmlArr.push('<div class="tip_popLayer tip_pSpace" style="width:80%">');
                    dialogHtmlArr.push('<p class="tip_editTCon">' + content + '</p>');
                    dialogHtmlArr.push('<div class="tip_editArea">');
                    if (btnCancel){
                        dialogHtmlArr.push(btnOk ? '<a href="javascript:;" class="tip_editBtn1" title="">' + btnOk + '</a>' : '');
                    }else{
                         dialogHtmlArr.push(btnOk ? '<a href="javascript:;" class="tip_editBtn1 tip_db" title="">' + btnOk + '</a>' : '');
                    }
                    if (btnOk){
                        dialogHtmlArr.push(btnCancel ? '<a href="javascript:;" class="tip_editBtn2" title="">' + btnCancel + '</a>' : '');
                    }else{
                        dialogHtmlArr.push(btnCancel ? '<a href="javascript:;" class="tip_editBtn2 tip_db" title="">' + btnCancel + '</a>' : '');
                    }
            ...
            // close click
            $('#' + dialogId + ' .tip_close').click(function() {
                if (isShowMask) {
                    document.ontouchmove = function(e){ return true;}
                }
            ...
            // ok callback
            if (typeof opts.ok == 'function') {
                $('#' + dialogId + ' .tip_editBtn1').click(function() {
                    opts.ok();
                });
            }
            ...
            // cancel callback
            if (typeof opts.cancel == 'function') {
                $('#' + dialogId + ' .tip_editBtn2').click(function() {
                    opts.cancel();
                });
            }

            if ($('#' + dialogId + ' .tip_editBtn1')[0]) {
                $('#' + dialogId + ' .tip_editBtn1').click(function() {$('#' + dialogId + ' .tip_close').click()});
            }
            if ($('#' + dialogId + ' .tip_editBtn2')[0]) {
                $('#' + dialogId + ' .tip_editBtn2').click(function() {$('#' + dialogId + ' .tip_close').click()});
            }

            // 自动关闭弹窗
            if (!opts.title && !btnOk && !btnCancel && autoClose) {
                autoClose = autoClose > 1 ? autoClose : 2000;
                setTimeout(function() {
        ...
        timerId: false,
        showLoading: function(display, waiting, autoClose) {                    //objGlobal.DIC.showLoading("block", "正在加载", false);
            var display = display || 'block';
            var autoClose = autoClose || false;
            waiting = waiting || '正在加载...';
            if (display == 'block') {
                objGlobal.DIC.dialog({id:'loading',content:waiting,noMask:true, autoClose:autoClose});
            } else {
                objGlobal.DIC.dialog({id:'loading'});
            }
        }
    }
},
...
//------------------------------------------------------------------------------
objJPEGEncoder = {                                                              //jpegEncoder处理生成jpg压缩格式
    ...
    init: function(){
        ...
        var ImageCompresser = {
            ...
            getImageBase64: function(img, opts){
                ...
                //ios平台针对大图做抽值处理
                if(mb_os.ios && ImageCompresser.isIosSubSample(img)){
                    _w = _w / 2;
                    _h = _h / 2;
                }
                //获取最终生成图片大小
                ...
                if (mb_os.android) {
                    var imgData = ctx.getImageData(0, 0, canvas.width, canvas.height),
                        encoder = new objJPEGEncoder.JPEGEncoder(quality * 100);
